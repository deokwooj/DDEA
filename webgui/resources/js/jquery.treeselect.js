(function($) {
  ...
  $.fn.treeselect = function(params) {
    ...
    params = $.extend({
      ...
      ,nodeclicked: null           /** action when a terminal node is clicked */
    }, params);
...
//console.log("TreeNode.prototype.loadAll  id <" + node.id + "> child count (" + i + ")");
...
//console.log("TreeNode.prototype.selectChildren <" + this.id + ">  state[" + state + "]  done[" + done + "]  child(" + child + ")");
...
//console.log("TreeNode.prototype.selectChildren : operation id<" + node.id + "> children [" + node.children.length + "]");
...
//console.log("TreeNode.prototype.setChecked title <" + this.title + "> children (" + this.children.length + ") checked : " + this.input.eq(0)[0].checked + " meta " + this.meta);
      }
...
//console.log("TreeNode.prototype.select title <" + this.title + "> children (" + this.children.length + ") state : " + state + " treenode-no-select [" +  this.input.hasClass('treenode-no-select') + "]");
    ...
    TreeNode.prototype.build_title = function(left) {
      ...
      if ((!this.root || this.showRoot) && this.title) {
        ...
        // Create a node link.
        this.nodeLink = $(document.createElement('a'))
                          .attr({
        ...
        if (this.has_children) 
        {
          ...
          this.link = $(document.createElement('div'))
                        .attr({'class': 'treeselect-title'})
                        .css('marginLeft', left + 'px')
                        .text(this.title)
                        .bind('click', {node: this}, function(event) {
                          event.preventDefault();
                          if (params.nodeclicked){
                            params.nodeclicked(event.data.node);
                          }
                        });
