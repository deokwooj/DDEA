        ...
        for idx1 in range(len(a)):
            ...
            for idx2 in range(idx1 + 1, len(a)):
                ...
                if FLATTEN:
                    ...
                    if isinstance(a[idx1], list) and isinstance(a[idx2], list):
                        pair_set.append([a[idx1]+a[idx2]][0])

                    elif isinstance(a[idx1], list) and not isinstance(a[idx2], list):
                        pair_set.append([list([a[idx2]])+a[idx1]][0])

                    elif not isinstance(a[idx1], list) and isinstance(a[idx2], list):
        ...
        for idx1 in a:
            ...
            for idx2 in b:
                ...
                if FLATTEN:
                    ...
                    elif isinstance(idx1, list) and not isinstance(idx2, list):
                        pair_set.append([idx1 + list([idx2])][0])

                    elif not isinstance(idx1, list) and isinstance(idx2, list):
...
def remove_dot(pname_):
    ...
    if isinstance(pname_, list):
        pname_list = []
        for name_ in pname_:

            if is_valid_uuid(name_):
                pname_list.append(name_.replace('-', '_'))
                continue
        ...
        if is_valid_uuid(pname_):
            return pname_.replace('-', '_')
