...
def pp_cluster_state_retrieval(tup):
    (obs, num_clusters, est_method) = tup
...
def bldg_obj_weather_convert(bldg_obj, sig_tag='avg', PARALLEL=False):
    ...
    if bldg_sigtag.data_weather_mat is not None and isinstance(bldg_sigtag.data_weather_mat, np.ndarray):
        ...
        wdata_mat, weather_dict = weather_convert(wdata_mat, wdata_name, conditions_dict, events_dict, PARALLEL=PARALLEL)
...
def _get_tick_symbol(tick_state_val, cause_labels, event, cond):
    ...
    for cause_label, state_val in iter_zip:
        symbol_out = list()
        if not isinstance(state_val, np.ndarray) and not isinstance(state_val, list):
            state_val = [state_val]
